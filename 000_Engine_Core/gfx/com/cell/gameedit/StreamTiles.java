...
public abstract class StreamTiles implements IImages, Runnable
{
	...
	private HashMap<String, IImage> imageKeys;
//	protected AtomicInteger			mode_ = new AtomicInteger(0);
	...
	protected void initImageKeys() {
		imageKeys = new HashMap<String, IImage>(img.Count);
		for (int i=0; i<img.Count; i++) {
			if (img.ClipsKey[i]!= null && !img.ClipsKey[i].isEmpty()) {
				imageKeys.put(img.ClipsKey[i], getImage(i));
			}
		}
	}
	...
	public IImage getImageWithKey(String key) {
		return imageKeys.get(key);
	}
...
//	}
	...
	public void render(IGraphics g, int Index, float PosX, float PosY, int Style) {
		IImage buff = images[Index];
		if (buff != null) {
	...
	protected void drawLoading(IGraphics g, float x, float y, float w, float h) 
	{
