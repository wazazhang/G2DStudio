...
//	} ImageTrans;
	...
	public static enum ImageAnchor implements ValueEnum<Integer>
	{
		L_T(0),
		C_T(1),
		R_T(2),
	...
	public static enum ImageTrans implements ValueEnum<Integer>
	{
		NONE(IGraphics.TRANS_NONE),
		R_90(IGraphics.TRANS_ROT90), 
		R_180(IGraphics.TRANS_ROT180),
		R_270(IGraphics.TRANS_ROT270),
		MIRROR(IGraphics.TRANS_MIRROR),
		MR_90(IGraphics.TRANS_MIRROR_ROT90), 
		MR_180(IGraphics.TRANS_MIRROR_ROT180),
		MR_270(IGraphics.TRANS_MIRROR_ROT270);
		
		final public int value;
		private ImageTrans(int v) {
			value = v;
		}
		@Override
		public Integer getValue() {
...
//	public void drawImage(IImage img, int x, int y, int transform, int blend_mode, float blend_alpha); 
	 ...
	 */
	public void drawImage(IImage img, float x, float y, int transform); 
...
//	一个组合精灵一帧可能有很多图片，一帧共用渲染模式。
	 ...
	 * @param transform 翻转方式
	 */
	public void drawImage(IImage img, float x, float y, float w, float h, int transform); 
...
//	用setBlendMode(int blend);改变渲染模式，不需要每画一张图片就改变一次Blend
	 ...
	 * @param y_dest 目标Y坐标。
	 */
	public void drawRegion(IImage src, int x_src, int y_src, int width, int height, int transform, float x_dest, float y_dest); 
	 ...
	 * @param transform 翻转方式
	 */
	public void drawRoundImage(IImage img, float x, float y, float width, float height, int transform); 
