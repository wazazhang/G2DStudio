 ...
 * every frame contian some part </br> every part is Images's index value </br>
 * every part has itself 2d coordinate x y </br>> every part has itself Flip
 * attribute </br>
 * 
 * @author yifeizhang
 * @since 2006-11-29
...
public class CAnimates extends CGroup {
	...
	public void addPart(boolean center) {
		for (int i = 0; i < images.getCount(); i++) {
			...
			if (center) {
				addPart(-images.getWidth(i) / 2, -images.getHeight(i) / 2, i,
						IGraphics.TRANS_NONE);
			} else {
				addPart(0, 0, i, IGraphics.TRANS_NONE);
			}
		}
	}
	 ...
	 * 
	 * @param px
	 *            part's x coordinate </br>�1�7
	 * @param py
	 *            part's y coordinate </br>�1�7
	 * @param tileid
	 *            part's images index value </br>
	 * @param trans
	 *            part's flip rotate paramenter
	 * */
	public void addPart(float px, float py, int tileid, int trans) {
		if (SubIndex >= SubCount) {
			System.err.println("Out Of Animate Max Count !");
			return;
		}
		if (SubIndex < SubCount) {
			...
			fixArea(SX[SubIndex], SY[SubIndex], SX[SubIndex] + SW[SubIndex],
	 ...
	 * 
	 * @param frame
	 *            frame id within animates</br>
	 * @param sub
	 *            part id within frame</br>
	 * @return image
	 */
	...
	public float[] getFrameBounds(int frame) {
		...
		for (int i = 0; i < SX.length; i++) {
			...
			bottom = Math
					.max(getFrameY(frame, i) + getFrameH(frame, i), bottom);
		}
	 ...
	 *            frame id </br>
	 * @param x
	 *            x on graphics surface</br>
	 * @param y
	 *            y on graphics surface</br>
	 */
	public void render(IGraphics g, int index, float x, float y) {
		for (int i = Frames[index].length - 1; i >= 0; i--) {
			int idx = Frames[index][i];
			images.render(g, STileID[idx], x + SX[idx], //
					y + SY[idx], //
					SFlip[idx]);
		}
	}
	 ...
	 *            part id </br>
	 * @param x
	 *            x on graphics surface </br>�1�7
	 * @param y
	 *            y on graphics surface </br>
	 */
	public void renderSub(IGraphics g, int index, int part, float x, float y) {
	...
	/**
	 * Draw one frame with specify frame id ignore part's coordinate, all part
	 * based zero point.</br>
	 * 
	 * @param g
	 *            graphics surface
	 ...
	 *            frame id
	 * @param x
	 *            x on graphics surface
	 * @param y
	 *            y on graphics surface
	 */
	public void renderSingle(IGraphics g, int index, float x, float y) {
		for (int i = Frames[index].length - 1; i >= 0; i--) {
			int idx = Frames[index][i];
			images.render(g, STileID[idx], x, //
					y, //
					SFlip[idx]);
		}
	}
	 ...
	 *            y on graphics surface
	 */
	public void renderSingleSub(IGraphics g, int index, int part, float x,
			float y) {
		int idx = Frames[index][part];
		images.render(g, STileID[idx], x, //
				y, //
