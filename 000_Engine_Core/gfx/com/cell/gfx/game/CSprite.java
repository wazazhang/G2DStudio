...
//	------------------------------------------------------------------------------------------
	...
	public float getFrameImageX(int anim,int frame,int sub){
		return animates.getFrameX(FrameAnimate[anim][frame], sub);
	}
	
	public float getFrameImageY(int anim,int frame,int sub){
		return animates.getFrameY(FrameAnimate[anim][frame], sub);
	}
	
	public float getFrameImageWidth(int anim, int frame, int sub) {
		return animates.getFrameW(FrameAnimate[anim][frame], sub);
	}
	
	public float getFrameImageHeight(int anim, int frame, int sub) {
...
//	------------------------------------------------------------------------------------------
	...
	{
			int mbx = (int)(spr.X+spr.getCurrentCD(type,sub).X1)/map.CellW-((spr.X+spr.getCurrentCD(type,sub).X1)<0?1:0);//
			int mby = (int)(spr.Y+spr.getCurrentCD(type,sub).Y1)/map.CellH-((spr.Y+spr.getCurrentCD(type,sub).Y1)<0?1:0);//
			int mbw = (int)(spr.X+spr.getCurrentCD(type,sub).X2)/map.CellW-((spr.X+spr.getCurrentCD(type,sub).X2)<0?1:0);//
			int mbh = (int)(spr.Y+spr.getCurrentCD(type,sub).Y2)/map.CellH-((spr.Y+spr.getCurrentCD(type,sub).Y2)<0?1:0);//
			if(map.IsCyc==false){
				for (int y=mby<0?0:mby; y<=mbh && y<map.getYCount(); y++) {
					for (int x=mbx<0?0:mbx; x<=mbw && x<map.getXCount(); x++) {
						if (map.layers[mapLayer].MatrixFlag[y][x] > 0) //
						{	
				...
				for (int y = mby; y <= mbh; y++) {
					for (int x = mbx; x <= mbw; x++) {
						...
						if (map.layers[mapLayer].MatrixFlag[cycX][cycY] > 0) //
						{
							if(CSprite.touch_SprFrameSub_CD(
									spr, anim, frame, type, sub,
...
//	------------------------------------------------------------------------------------------
	...
	public float getCDHeight(){
		return collides.w_height;
	}
	public float getCDWidth(){
...
//	------------------------------------------------------------------------------------------
	...
	public void render(IGraphics g,float x,float y) {
		if (OnScreen){
	...
	public void render(IGraphics g,float x,float y, int blend_mode, float blend_alpha) {
		if (OnScreen){
			if ( (CurAnimate < FrameAnimate.length) && (CurFrame < FrameAnimate[CurAnimate].length) ) {
				g.pushBlendMode();
				try {
	...
	public void render(IGraphics g,float x,float y, int anim, int frame) {
		if (OnScreen){
	...
	public void render(IGraphics g,float x,float y, int anim, int frame, int blend_mode, float blend_alpha) {
		if (OnScreen){
			if ( (anim < FrameAnimate.length) && (frame < FrameAnimate[anim].length) ) {
				g.pushBlendMode();
				try {
