...
class CGraphicsImage implements IGraphics 
{
	...
	final  public  void drawLine(float arg0, float arg1, float arg2, float arg3) {
		m_graphics2d.drawLine((int)arg0, (int)arg1, (int)arg2, (int)arg3);
	}

	final  public void drawRect(float arg0, float arg1, float arg2, float arg3) {
		m_graphics2d.drawRect((int)arg0, (int)arg1, (int)arg2-1, (int)arg3-1);
	}
	...
	final  public  void fillRect(float arg0, float arg1, float arg2, float arg3) {
		m_graphics2d.fillRect((int)arg0, (int)arg1, (int)arg2, (int)arg3);
	}
	...
	private Stack<AffineTransform> stack_matrix = new Stack<AffineTransform>();
	
	@Override
	public void popTransform() {
		AffineTransform tx = stack_matrix.pop();
		if (tx != null) {
			m_graphics2d.setTransform(tx);
		}
	}

	@Override
	public void pushTransform() {
		 stack_matrix.push(m_graphics2d.getTransform());
	}
	...
	final public void setBlendMode(int blend, float alpha) {
		if (blend != IGraphics.BLEND_MODE_NORMAL) {
	...
	final public void transform(int transform, float width, float height)
	{
		switch (transform) 
		{
		case TRANS_ROT90: {
	...
	final  public  void drawImage(IImage src, float x, float y, int transform) 
	{
		Image img = ((CImage)src).getSrc();
        
        if (transform == 0)
        {      	 
        	m_graphics2d.drawImage(img, (int)x, (int)y, null);
        }
        else
        {
	...
	final public void drawImage(IImage src, float x, float y, float w, float h, int transform) 
	{
		Image img = ((CImage)src).getSrc();
        ...
        else
        {
			...
			m_graphics2d.drawImage(img, 0, 0, (int)w, (int)h, null);
			m_graphics2d.setTransform(savedT);
		}
	...
	final  public void drawRoundImage(IImage src, float x, float y, float width, float height, int transform) 
	{
		Image img = ((CImage)src).getSrc();

		Rectangle rect = m_graphics2d.getClipBounds();

		m_graphics2d.clipRect((int) x, (int) y, (int) width, (int) height);
		...
		for (int dx = 0; dx < width;) {
			for (int dy = 0; dy < height;) {
				m_graphics2d.drawImage(img, (int)x + dx, (int)y + dy, null);
				AScreen.FrameImageDrawed++;
				dy += h;
			}
			dx += w;
	...
	final  public void drawRegion(IImage src, int x_src, int y_src, int width, int height, int transform, float x_dst, float y_dst)
    {
		Image img = ((CImage)src).getSrc();
	...
	protected final Composite getCompositeFrom(int blend_mode, float blend_alpha)
	{
		...
		if (blend_mode == IGraphics.BLEND_MODE_SCREEN)
		{
		...
		{
			composite = java.awt.AlphaComposite.SrcOver.derive(blend_alpha);
		}		
		return composite;
	}
...
//	-------------------------------------------------------------------------------------------------------------------------
	...
	@Deprecated
	public void drawString(String str, float x, float y, int shandowColor, int shandowX, int shandowY) {
		throw new NotImplementedException();
	}
	@Deprecated
