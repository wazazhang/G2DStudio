...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/**
	 * find src array of dst range
	 * @param src
	 * @param dst
	 * @return
	 */
	public static<T> int arrayIndexOf(T[] src, T[] dst) 
	{
		...
		int dlen = src.length - dst.length;
		for (int i = 0; i <= dlen; i++) 
		{
			boolean isok = true;
			for (int j = 0; j < dst.length; j++) 
			{
				if (!src[i + j].equals(dst[j])) {
					isok = false;
					break;
				}
			}
	 ...
	 * @return
	 */
	public static int arrayIndexOf(Object src, Object dst) 
	{
		int src_length = Array.getLength(src);
		...
		int dlen = src_length - dst_length;
		for (int i = 0; i <= dlen; i++) 
		{
			boolean isok = true;
			for (int j = 0; j < dst_length; j++) 
			{
				Object sobj = Array.get(src, i + j);
				Object dobj = Array.get(dst, j);
				if (!sobj.equals(dobj)) {
					isok = false;
					break;
				}
...
//	---------------------------------------------------------------------------------------------------------------
	...
	static public int string4CharToIntLittle(String str)
	{
		int ret = 0;
	...
	static public String intLittle4String4Char(int value)
	{
		char c3 = (char)((value) & 0x000000FF);
		char c2 = (char)((value>>8) & 0x000000FF);
		char c1 = (char)((value>>16) & 0x000000FF);
		char c0 = (char)((value>>24) & 0x000000FF);
		return c3 + "" + c2 + "" + c1 + "" + c0;
	}
...
//	} 
	...
	static public String toHexU32(int v)
	{
		return snapStringLeftSize(Integer.toHexString(v), 8, '0');
	}
	
	static public String toHexU64(long v)
	{
		return snapStringLeftSize(Long.toHexString(v), 16, '0');
	}
...
//--------------------------------------------------------------------------------------------------------    
     ...
     * @return
     */
    public static String snapStringLeftSize(Object src, int length, char blank) 
    {
    	return snapStringLeftSize(src.toString(), length, blank);
    }
     ...
     * @return
     */
    public static String snapStringLeftSize(String src, int length, char blank) 
    {
    	if (src.length()<length) {
    		for (int i=length-src.length()-1; i>=0; --i) {
    			src = blank + src;
...
//	--------------------------------------------------------------------------------------------------------------------------
	...
	static public String timeYYMMDDHHMMSS(long timeInMillis)
	{
		...
		return 
				snapStringLeftSize(dsts[0], 4, '0') + 
				snapStringLeftSize(dsts[1], 2, '0') + 
				snapStringLeftSize(dsts[2], 2, '0') + 
				snapStringLeftSize(dsts[3], 2, '0') + 
				snapStringLeftSize(dsts[4], 2, '0') + 
				snapStringLeftSize(dsts[5], 2, '0');
	}
}
