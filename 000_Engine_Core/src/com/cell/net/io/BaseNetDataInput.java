...
//	-----------------------------------------------------------------------------------------------
	...
	public <T> T readExternal(Class<T> cls) throws IOException {
		...
		if (type != 0) {
			try {
				Class<?> true_type = this.factory.getMessageClass(type);
				@SuppressWarnings("unchecked")
				T data = (T)true_type.newInstance();
	...
	public <T> T readMutual(Class<T> cls) throws IOException {
		int msg_type = readInt();
		if (msg_type != 0) {
			try {
				Class<?> true_type = this.factory.getMessageClass(msg_type);
				@SuppressWarnings("unchecked")
				T data = (T)true_type.newInstance();
