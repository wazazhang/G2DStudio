...
//		sb.append("	public static function new_" + m_name + "() : " + c_name + " {return new " + c_name + "();}\n");
	...
	protected void genCodecField(Class<?> msg, Field f, StringBuilder read, StringBuilder write)
	{
		...
		// boolean -----------------------------------------------
		if (f_type.equals(boolean.class) || f_type.equals(Boolean.class)) {
			read.append("		" + f_name + " = input.readBoolean();\n");
			write.append("		output.writeBoolean(" + f_name + ");\n");
		}
		else if (f_type.equals(boolean[].class) || f_type.equals(Boolean[].class)) {
...
//	------------------------------------------------------------------------------------------------------------------
	...
	protected String genMsgClass(Class<?> msg, int msg_type) 
	{
		...
		for (Field f : msg.getDeclaredFields()) 
		{
			...
			{
				Object value = genMsgStaticValue(msg, f);
				// message is enum
	...
	protected Object genMsgStaticValue(Class<?> msg, Field f)
	{
		Object value = null;
		try {
			value = f.get(null);
		} catch (Exception e) {}
		if (msg.isEnum() || CharSequence.class.isAssignableFrom(f.getType()))
		{
			return "\"" + value + "\"";
		}
