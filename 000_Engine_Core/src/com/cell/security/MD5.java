...
public class MD5
 {
	...
	public static final String getMD5cyc(File str) {
		return getMD6(str);
	}
	public static final String getMD5cyc(byte[] data) {
		return getMD6(data);
	}
	public static final String getMD5cyc(ByteBuffer data) {
		return getMD6(data);
	}
	...
	public static final String getMDX(ByteBuffer data, int type) {
		if (type==COVERT_TYPE_MD5) {
			return getMD5(data);
		}else if (type==COVERT_TYPE_MD6) {
			return getMD5cyc(data);
	...
	public static final String getMDX(File data, int type) {
		if (type==COVERT_TYPE_MD5) {
			return getMD5(data);
		}else if (type==COVERT_TYPE_MD6) {
			return getMD5cyc(data);
...
//	------------------------------------------------------------------------------------------------------------------------------
	...
	private static String _md5str(byte tmp[]) 
	{
		// 用字节表示就是 16 个字节
	...
	private static String md5(ByteBuffer source)
	{
		String s = null;
	...
	private static String md5(File source)
	{
		String s = null;
    ...
    private static String _md6str(String str) {
    	String str_ret = "";
    ...
    private static String md6(File source)
    {
    ...
    private static String md6(ByteBuffer source)
    {
    	String str = md5(source);
	...
	public static String getMD6(ByteBuffer source){
		return md6(source);
	}
    ...
    public static String getMD6(File source) {
    	return md6(source);
    }
...
//    }
    ...
    private static String processVerbos(int verbos, long size, long date, String name) {
    	String dst = "";
    		...
    		StringBuilder output) throws Exception
    {
		...
		{
//			if (filters != null && !filters.acceptPath(srcFile)) {
...
//			}
			...
			for (int l=0; l<files.length; l++)
			{
				...
				else if (files[l].isFile()) {
					if (filters != null && !filters.acceptFile(files[l])) {
						ret.getValue().incrementAndGet();
						continue;
					}
					String dst = MD5.getMDX(files[l], CoverType);
					dst += processVerbos(verbos,
							files[l].length(),
							files[l].lastModified(), 
							files[l].getPath());
					System.out.println(dst);
					output.append(dst + "\n");
				}
		...
		{
			String dst = getMDX(srcFile, CoverType);// + (IsOutputSrc ? " : " + srcFile.getPath() : "");
			dst += processVerbos(verbos, srcFile.length(), srcFile.lastModified(), srcFile.getPath());
			System.out.println(dst);
			output.append(dst + "\n");
    		...
    		StringBuilder output) throws Exception
    {
		...
		{
//			if (filters != null && !filters.acceptPath(srcDir)) {
//				ret.getValue().incrementAndGet();
//				return;
//			}
    		...
    		Pair<AtomicInteger, AtomicInteger> ret) throws Exception
    {
		...
		{
			byte[] data = CFile.readData(srcTextFile);
			String text = null;
...
//		ArrayList<Pattern> filters_dec = null;
	...
	private static void mainOld(String[] args) throws Exception
	{
		...
		else if (args[1].startsWith("-f:")) 
		{
			...
			if (args[0].startsWith("-md5")) {
				System.out.println(MD5.getMD5(file) + (isShowSrc ? " : " + file.getPath() : ""));
			} else {
				System.out.println(MD5.getMD5cyc(file) + (isShowSrc ? " : " + file.getPath() : ""));
			}
			return;
		} 
		...
		else if (args[1].startsWith("-dir:"))
		{
			...
			for (int l=0; l<files.length; l++)
			{
				...
				String dst = "";
				if (args[0].startsWith("-md5")) {
					dst = MD5.getMD5(files[l])+(isShowSrc ? " : " + files[l].getPath() : "");
				} else {
					dst = MD5.getMD5cyc(files[l])+(isShowSrc ? " : " + files[l].getPath() : "");
				}
				System.out.println(dst);
				outputs += dst + "\n";
			}
		...
		else if (args[1].startsWith("-cmp:"))
		{
			...
			if (cmp.isDirectory())
			{
				...
				for (int l=0; l<files.length; l++)
				{
					...
					if (args[0].startsWith("-md5")) {
						outputs += MD5.getMD5(files[l]);
					} else {
						outputs += MD5.getMD5cyc(files[l]);
					}
				}
			...
			{
				if (args[0].startsWith("-md5")) {
					outputs += MD5.getMD5(cmp);
				} else {
					outputs += MD5.getMD5cyc(cmp);
				}
			}
