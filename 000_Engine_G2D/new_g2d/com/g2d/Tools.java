...
//	--------------------------------------------------------------------------------
	...
	public static AnimateCursor createSpriteCursor(String name, CSprite cspr, int anim)
	{
		...
		for (int f = 0; f < frame_count; f++) {
			BufferedImage buff = createImage((int)max_bounds.getWidth(), (int)max_bounds.getHeight());
			Graphics2D g2d = buff.createGraphics();
			cspr.render(g2d, -max_bounds.X1, -max_bounds.Y1, anim, f);
			g2d.dispose();
			array[f] = buff;
		}
		return Engine.getEngine().createCursor(array, (int)-max_bounds.X1, (int)-max_bounds.Y1, name);
	}
...
//	--------------------------------------------------------------------------------
	...
	static public BufferedImage createImage(CSprite spr, int anim, int frame)
	{
		...
		{	
			...
			BufferedImage buf = Engine.getEngine().createImage((int)bounds.getWidth(), (int)bounds.getHeight());
			Graphics2D g = (Graphics2D)buf.createGraphics();
			try {
