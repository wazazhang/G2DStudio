...
import com.g2d.annotation.PropertyNotNull;
...
//	 public
	...
	/** 当前坐标系的 rectangle */
	@Property(value="Bounds")@PropertyNotNull()
...
//	--------------------------------------------------------------------------------------------------------------------------------------------
	...
	private int debug_render_time = 0;
	private int last_render_time = 0;
	protected void increaseFrameRenderTime(int d) {
		debug_render_time += d;
	}
	public int getFrameRenderTime() {
		return last_render_time;
	}
	protected void startFrameRecorder() {
		debug_render_time = 0;
	}
	protected void endFrameRecorder() {
		last_render_time = debug_render_time;
	}
//	--------------------------------------------------------------------------------------------------------------------------------------------
...
//	screen position with root
	...
	final public double screenToLocalX(double x) {
		return x - screen_x;
	}

	final public double screenToLocalY(double y) {
		return y - screen_y;
	}

	final public double localToScreenX(double x) {
		return screen_x + x;
	}

	final public double localToScreenY(double y) {
		return screen_y + y;
	}
	...
	/** 
	 * 获取当前节点的某位置相对于dst节点的位置
	 * @param dst
	 * @return
	 */
	final public double convertCoordinateX(DisplayObject dst, double x) 
	{
		double sx = localToScreenX(x);
		return dst.screenToLocalX(sx);
	}
	final public double convertCoordinateY(DisplayObject dst, double y) 
	{
		double sy = localToScreenY(y);
		return dst.screenToLocalY(sy);
	}
...
//	---------------------------------------------------------------------------------------------------------------------------------------
	...
	void onUpdate(DisplayObjectContainer parent) 
	{
		...
		this.mouse_x = (int)screenToLocalX(root.getMouseX());
		this.mouse_y = (int)screenToLocalY(root.getMouseY());
...
//	------------------------------------------------------------------
	...
	private LinkedHashSet<Action> actions = null;
	
	protected void onActionStarted(Action act) {
	...
	protected void onActionUpdated(Action act) {
	...
	private void updateActions(Graphics2D g) {
		if (actions == null || actions.isEmpty()) {
			return;
		}
		for (Action a : new ArrayList<Action>(actions)) {
			a.update(g);
			onActionUpdated(a);
			if (a.isEnd()) {
				actions.remove(a);
	...
	final public boolean runAction(Action act) {
		if (actions == null) {
			actions = new LinkedHashSet<Action>();
		}
		act.start(this);
		boolean ret = actions.add(act);
		onActionStarted(act);
		return ret;
	}

	final public boolean stopAction(Action act) {
		if (actions != null && actions.contains(act)) {
			actions.remove(act);
			act.stop();
			onActionStoped(act);
			return true;
