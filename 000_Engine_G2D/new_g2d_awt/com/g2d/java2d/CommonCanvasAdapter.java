...
//	game stage
	...
	private float 						screenOffsetX = 0, screenOffsetY= 0;
...
//	}
	...
	public void setScreenScale(float scale) {
		...
		this.setScreenOffest(screenOffsetX, screenOffsetY);
	}
	...
	@Override
	public void setScreenOffest(float x, float y) {
		x = Math.max(x, 0);
		y = Math.max(y, 0);
		x = Math.min(x, getStageWidth() - getStageWidth() / screenScale);
		y = Math.min(y, getStageHeight() - getStageHeight() / screenScale);
		this.screenOffsetX = x;
		this.screenOffsetY = y;
	}
	@Override
	public float getScreenOffestX() {
		return screenOffsetX;
	}
	@Override
	public float getScreenOffestY() {
...
//		GraphicsConfiguration gc = ((Graphics2D)g).getDeviceConfiguration();
		java.awt.geom.AffineTransform trans = g2d.getTransform();
		try 
		{
			...
			fixMouse() ;
			queryKey();
			size_rate_x = getMouseSizeRateW(stageWidth)/screenScale;
			size_rate_y = getMouseSizeRateH(stageHeight)/screenScale;
			nextCursor = null;
