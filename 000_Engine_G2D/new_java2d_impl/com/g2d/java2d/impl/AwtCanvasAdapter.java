...
//		System.out.println(getClass().getName() + " : finalize");
	...
	protected void updateStage(java.awt.Graphics2D g, Stage currentStage)
	{
		...
		} else if (vm_buffer.contentsLost() ) {
			destory_vm_buffer();
			vm_buffer = create_vm_buffer(gc);
		...
		if (currentStage != null)
		{
			...
			try {
				...
				awt_g.pushTransform();
				awt_g.translate(getScreenOffestX(), getScreenOffestY());
				awt_g.scale(getScreenScale(), getScreenScale());
				awt_g.translate(-getScreenOffestX() * getScreenScale(), -getScreenOffestY() * getScreenScale());
				currentStage.onRender(this, awt_g);
				awt_g.popTransform();
				currentStage.onRenderScreen(awt_g);
			} finally {
				g2d.dispose();
			}
		}
