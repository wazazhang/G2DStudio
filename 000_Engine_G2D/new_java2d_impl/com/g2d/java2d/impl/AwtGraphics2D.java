...
import com.g2d.java2d.impl.composite.BlendScreen;
...
public class AwtGraphics2D extends Graphics2D
{
	...
	private static java.awt.Font global_awtfont;
	...
	private Stack<Font> 							stack_font		= new Stack<Font>();

	private AlphaComposite 	blend_NORMAL = AlphaComposite.SrcOver;
	private AlphaComposite 	blend_SCREEN = AlphaComposite.SrcIn;
	...
	protected AwtGraphics2D(java.awt.Graphics2D g2d)
 {
		...
		if (global_awtfont != null) {
			this.setFont(new AwtFont(global_awtfont));
		} else {
			this.setFont(new AwtFont(g2d.getFont()));
		}
	}
	...
	public void setBlendMode(int blend, float alpha) {
		if (AwtEngine.ENABLE_BLEND) {
			...
			case BLEND_MODE_NORMAL:
				g2d.setComposite(blend_NORMAL.derive(alpha));
				break;
			case BLEND_MODE_SCREEN:
				g2d.setComposite(blend_SCREEN.derive(alpha));
				break;
			default:
				g2d.setComposite(blend_NORMAL.derive(alpha));
				break;
...
//	-------------------------------------------------------------------------------------------------------------------------
	...
	public void pushFont() {
		stack_font.push(cur_font);
	}
	public void popFont() {
		this.setFont(stack_font.pop());
	}
...
//		if (global_awtfont != null) {
//			this.cur_font = AwtEngine.wrap(global_awtfont.deriveFont(font.getStyle(), font.getSize()));
//		} else {
			this.cur_font = font;
			g2d.setFont(((AwtFont)cur_font).getFont());
			this.cur_font_h = g2d.getFontMetrics().getHeight();
//		}
	}
	public void setFont(String name) {
		AwtFont font = new AwtFont(new java.awt.Font(name, java.awt.Font.PLAIN, cur_font.getSize()));
		this.setFont(font);
	}
	...
	public void setFontSize(int size) {
		java.awt.Font awtfont = ((AwtFont)cur_font).getFont();
		this.setFont(new AwtFont(awtfont.deriveFont((float)size)));
	}
...
//			g2d.fillRect((int)x, (int)y, (int)rect.getWidth(), (int)rect.getHeight());
//			setColor(cur_color);
//		}
...
//	-------------------------------------------------------------------------------------------------------------------------
	...
	public boolean drawImage(Image img, float x, float y) {
		g2d.translate(x, y);
		g2d.drawImage(((AwtImage)img).getSrc(), 0, 0, null);
		g2d.translate(-x, -y);
		return true;
	}
	public boolean drawImage(Image img, float x, float y, float width, float height) {
		g2d.translate(x, y);
		g2d.drawImage(((AwtImage)img).getSrc(), 0, 0, (int)width, (int)height, null);
		g2d.translate(-x, -y);
		return true;
	}
	public boolean drawImage(Image img, 
		...
		return g2d.drawImage(((AwtImage)img).getSrc(),
				(int)dx1, (int)dy1, (int)dx2, (int)dy2,
				sx1, sy1, sx2, sy2, null);
	}
	
	@Override
	public void drawImage(IImage src, float x, float y, int transform) 
	{
	...
	@Override
	public void drawImage(IImage src, float x, float y, float w, float h, int transform) {
		BufferedImage img = ((AwtImage) src).getSrc();
		java.awt.geom.AffineTransform savedT = g2d.getTransform();
		try {
			g2d.translate(x, y);
			transform(transform, w, h);
			g2d.drawImage(img, 0, 0, (int)w, (int)h, null);
		} finally {
			g2d.setTransform(savedT);
		}
	}
