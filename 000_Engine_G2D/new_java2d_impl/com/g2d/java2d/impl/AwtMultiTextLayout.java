...
//	显示
	...
	private boolean 			is_antialias	= true;
...
//	----------------------------------------------------------------------------------------------------------------
	...
	public void setAntialias(boolean enable) {
		if (enable != is_antialias) {
			is_antialias = enable;
			if (textChange != null) {
				textChange.reset();
			} else {
				textChange = new TextChanges().reset();
			}
		}
	}
...
//		render_shadow_color = shadow_color;
	...
	private void resetText(Graphics2D g2d, TextChanges change)
	{
			...
			change.text.equals(this.text) == false)
		{
			...
			this.render_antialiasing = is_antialias;
...
//	------------------------------------------------------------------------------------------------------------------------
	...
	private class TextLine 
	{
		...
		float				render_shadow_alpha = 0;
		int					render_shadow_color	= 0;
		final int 			width;
		final int 			height;
