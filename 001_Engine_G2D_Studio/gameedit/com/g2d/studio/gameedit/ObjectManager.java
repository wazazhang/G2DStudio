...
import com.g2d.studio.gameedit.dynamic.util.AvatarTreeView;
import com.g2d.studio.gameedit.dynamic.util.EffectTreeView;
import com.g2d.studio.gameedit.dynamic.util.ItemListTreeView;
import com.g2d.studio.gameedit.dynamic.util.ShopItemListTreeView;
import com.g2d.studio.gameedit.dynamic.util.ShopItemTreeView;
...
import com.g2d.studio.gameedit.template.util.SkillTreeView;
import com.g2d.studio.gameedit.template.util.XLSItemManagerTree;
import com.g2d.studio.gameedit.template.util.XLSShopItemManagerTree;
import com.g2d.studio.gameedit.template.util.XLSSkillManagerTree;
import com.g2d.studio.gameedit.template.util.XLSUnitManagerTree;
...
//			XLSColumns.getXLSColumns(studio.xls_tpet.getInputStream());
		this.player_xls_columns = studio.xls_tplayer.GetColumns();
		this.pet_xls_columns = studio.xls_tpet.GetColumns();
	}
	...
	public ArrayList<ObjectManagerTree<?, ?>> getIconPages() {
		...
		pages.remove(managers.get(DAvatar.class));
		pages.remove(managers.get(DEffect.class));
		pages.add(managers.get(DAvatar.class));
		pages.add(managers.get(DEffect.class));
		return pages;
	}
...
//	------------------------------------------------------------------------------------------------------------------------
	...
	public <T extends ObjectNode<?>> boolean saveObject(T obj) {
		ObjectTreeView<?,?> page = getPage(obj.getClass());
		if (page.node_type.equals(obj.getClass())) {
			Object o = page.saveSingle(obj);
			return o != null;
		}
		return false;
	}
		...
		try{
			...
			if (row!=null) {
				if (node_type.equals(XLSUnit.class)) {
					node = (node_type.cast(new XLSUnit(row, data)));
				} else if (node_type.equals(XLSItem.class)) {
					node = (node_type.cast(new XLSItem(row, data)));
				} else if (node_type.equals(XLSShopItem.class)) {
					node = (node_type.cast(new XLSShopItem(row, data)));
				} else if (node_type.equals(XLSSkill.class)) {
					node = (node_type.cast(new XLSSkill(row, data)));
				}
			} else {
