...
import com.g2d.studio.gameedit.dynamic.util.ShopItemListTreeView.ShopItemListGroup;
...
final public class DShopItemList extends DynamicNode<TShopItemList>
{
	...
	public DShopItemList(ShopItemListTreeView f, int id, String name) throws Exception {
		super(f, id, name);
		this.factory = f;
	}
	
	public DShopItemList(ShopItemListTreeView f, TShopItemList data) {
		super(data, Integer.parseInt(data.getID()), data.name);
		this.factory = f;
	}
			...
			if (e.getSource() == clone) {
				if (node.getParent() instanceof ShopItemListGroup) {
					try {
						...
						DShopItemList		effect	= new DShopItemList(
								factory, factory.idf().createID(),
								src.name+" Copy");
