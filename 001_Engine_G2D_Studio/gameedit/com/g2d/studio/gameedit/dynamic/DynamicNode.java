...
public abstract class DynamicNode<T extends RPGObject> extends ObjectNode<T>
{
	 ...
	 * @param name
	 */
	public DynamicNode(IDynamicIDFactory<?> factory, int id, String name, Object... args) throws Exception {
		if (factory.idf().containsID(id)) {
			throw new Exception("ID冲突 - " + id);
		}
		this.id 		= id;
		this.name 		= name;
...
//	}
	...
	public boolean resetID(IDynamicIDFactory<?> idf, int newID) {
		if (idf.idf().swapID(this.id, newID)) {
			this.bind_data.setID(idf, newID+"");
			this.id = newID;
			return true;
		}
		return false;
