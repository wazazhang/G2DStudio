...
public class EffectEditor extends JSplitPane implements ActionListener, ListSelectionListener
{
	...
	public EffectEditor(TEffect e) 
	{
		...
		// init left
		{
			...
			layers.setMinimumSize(new Dimension(100, 400));
			layers.setPreferredSize(new Dimension(100, 400));
...
//		-------------------------------------------------------------------------------------------------------------------------------
		...
		class PageAppearance extends PropertyPage implements ActionListener
		{
			...
			class AppearanceSprite extends AppearancePage
			{
				...
				JButton				image_brwoser_effect_btn	= new JButton("浏览 Effect");
				JButton				image_brwoser_actor_btn 	= new JButton("浏览 Actor");
				JButton				image_brwoser_avatar_btn	= new JButton("浏览 Item");
				JLabel 				image_view					= new JLabel();
				...
				{
					tools.add(image_brwoser_effect_btn);
					tools.add(image_brwoser_actor_btn);
					tools.add(image_brwoser_avatar_btn);
...
//					add(south, BorderLayout.SOUTH);
					...
					image_brwoser_effect_btn.addActionListener(this);	
					image_brwoser_actor_btn.addActionListener(this);
					image_brwoser_avatar_btn.addActionListener(this);			
				}
				...
				public void actionPerformed(ActionEvent e)
				{
					...
					CPJResourceType res_type = null;
					ParticleResourceType res_type_p = null;
					if (e.getSource() == image_brwoser_effect_btn) {
						res_type = CPJResourceType.EFFECT;
						res_type_p = ParticleResourceType.EFFECT;
					}
					else if (e.getSource() == image_brwoser_actor_btn) {
						res_type = CPJResourceType.ACTOR;
						res_type_p = ParticleResourceType.ACTOR;
					}
					else if (e.getSource() == image_brwoser_avatar_btn) {
						res_type = CPJResourceType.AVATAR;
						res_type_p = ParticleResourceType.AVATAR;
					}
					if (res_type != null)
					{
						CPJSprite spr = new CPJResourceSelectDialog<CPJSprite>(
								this, res_type).showDialog();
						if (spr != null) {
							if (appearance == null) {
							...
							appearance.res_type = res_type_p;
...
//		-------------------------------------------------------------------------------------------------------------------------------
		...
		class PageScene extends PropertyPage implements KeyListener
		{
			...
			JLabel		particles_delay_frame	= new JLabel("每隔多少帧释放一次粒子");
			JSpinner	particles_delay_frame_v	= new JSpinner(new SpinnerNumberModel(0, 0, Integer.MAX_VALUE, 1));
			...
			public PageScene() 
			{
				int sx = 20, sy = 20;
				{
					...
					particles_delay_frame	.setBounds(sx, sy, 200, 24); sy += 25;
					particles_delay_frame_v	.setBounds(sx, sy, 200, 24); sy += 25;
					super.add(particles_delay_frame);
					super.add(particles_delay_frame_v);
			...
			void setData(Layer layer)
			{
				...
				particles_delay_frame_v	.setValue(layer.particles_delay_frame);
			...
			void getData(Layer layer)
			{
				...
				layer.particles_delay_frame	= Parser.castNumber(particles_delay_frame_v.getValue(), Integer.class);
...
//		-------------------------------------------------------------------------------------------------------------------------------
		...
		class PageOrigin extends PropertyPage implements ActionListener
		{
			...
			class Rectangle extends OriginShapeType
			{
				...
				JCheckBox isBorder = new JCheckBox("边缘");
				JLabel lbl_border_range = new JLabel("边缘±浮动范围");
				JSpinner border_range = new JSpinner(new SpinnerNumberModel(0, -Float.MAX_VALUE, Float.MAX_VALUE, 1f));
				...
				public Rectangle()
				{
					...
					isBorder.setBounds(0,  100, 80, 24);
					lbl_border_range.setBounds(0,  125, 80, 24);
					border_range	.setBounds(80, 125, 80, 24);
					...
					super.add(isBorder);
					super.add(lbl_border_range);
					super.add(border_range);
				}
				...
				public void setShape(OriginShape shape) {
					if (shape instanceof OriginShape.Rectangle) {
						...
						w.setValue(s.w);
						h.setValue(s.h);
						isBorder.setSelected(s.isBorder);
						border_range.setValue(s.broderRange);
					}
				}
				...
				public OriginShape getShape() {
					...
					ret.isBorder = isBorder.isSelected();
					ret.broderRange = Parser.castNumber(border_range.getValue(), Float.class);
					return ret;
				}
			}
...
//	--------------------------------------------------------------------------------------------------------------------
	...
	static class LineEdit extends JPanel implements G2DListItem, ActionListener, Comparable<LineEdit>
	{
		...
		JSpinner 	position_v		= new JSpinner(new SpinnerNumberModel(
				0d, 0d, 100d, 1f));

		JCheckBox 	enable_alpha	= new JCheckBox("Alpha变化");
		JSpinner 	alpha_v			= new JSpinner(new SpinnerNumberModel(
				1d, 0d, 1d, 0.1d));
...
//		JSpinner 	color_v			= new JSpinner(new SpinnerNumberModel(0d, 0d, 100d, 1f));
		...
		JSpinner 	size_xv			= new JSpinner(new SpinnerNumberModel(
				1d, -Float.MAX_VALUE, Float.MAX_VALUE, 0.1f));
		JSpinner 	size_yv			= new JSpinner(new SpinnerNumberModel(
				1d, -Float.MAX_VALUE, Float.MAX_VALUE, 0.1f));

		JCheckBox 	enable_spin		= new JCheckBox("旋转变化");
		JSpinner 	spin_v			= new JSpinner(new SpinnerNumberModel(
				0, -Float.MAX_VALUE, Float.MAX_VALUE, 1d));

		JCheckBox	 end_size		= new JCheckBox("END缩放变化");
		JSpinner 	end_size_xv		= new JSpinner(new SpinnerNumberModel(
				1d, -Float.MAX_VALUE, Float.MAX_VALUE, 0.1f));
		JSpinner 	end_size_yv		= new JSpinner(new SpinnerNumberModel(
				1d, -Float.MAX_VALUE, Float.MAX_VALUE, 0.1f));
		JCheckBox 		end_spin	= new JCheckBox("END旋转变化");
		JSpinner 	end_spin_v		= new JSpinner(new SpinnerNumberModel(
				0, -Float.MAX_VALUE, Float.MAX_VALUE, 1d));
		...
		public LineEdit(TimeNode time_node, G2DList<LineEdit> list) 
		{
			...
			int sx = 20, sy = 20;
			{
				...
				size_xv			.setBounds(sx + 0,   sy, 100, 24);
				size_yv			.setBounds(sx + 100, sy, 100, 24);
				sy += 25;
				...
				end_size		.setBounds(sx + 0,   sy, 200, 24); sy += 25;
				end_size_xv		.setBounds(sx + 0,   sy, 100, 24);
				end_size_yv		.setBounds(sx + 100, sy, 100, 24);
				sy += 25;
				super.add(end_size);
				super.add(end_size_xv);
				super.add(end_size_yv);
			
				end_spin		.setBounds(sx + 0,   sy, 200, 24); sy += 25;
				end_spin_v		.setBounds(sx + 0,   sy, 200, 24); sy += 25;
				super.add(end_spin);
				super.add(end_spin_v);
				
			}
			
			enable_size.addActionListener(this);
			enable_spin.addActionListener(this);
			end_size.addActionListener(this);
			end_spin.addActionListener(this);
		...
		public void actionPerformed(ActionEvent e) 
		{
			size_xv		.setEnabled(enable_size.isSelected());
			size_yv		.setEnabled(enable_size.isSelected());
			spin_v		.setEnabled(enable_spin.isSelected());
			
			end_size_xv	.setEnabled(end_size.isSelected());
			end_size_yv	.setEnabled(end_size.isSelected());
			end_spin_v	.setEnabled(end_spin.isSelected());
		...
		void setData()
		{
			...
			size_xv		.setValue(time_node.scale_x);
			size_yv		.setValue(time_node.scale_y);
			end_size_xv	.setValue(time_node.scale_end_x);
			end_size_yv	.setValue(time_node.scale_end_y);
			
			spin_v		.setValue(Math.toDegrees(time_node.spin));
			end_spin_v	.setValue(Math.toDegrees(time_node.spin_end));
			...
			end_size	.setSelected(time_node.end_size);
			end_spin	.setSelected(time_node.end_spin);
		...
		void getData()
		{
			...
			time_node.scale_x		= Parser.castNumber(size_xv.getValue(), Float.class);
			time_node.scale_y		= Parser.castNumber(size_yv.getValue(), Float.class);
			time_node.scale_end_x	= Parser.castNumber(end_size_xv.getValue(), Float.class);
			time_node.scale_end_y	= Parser.castNumber(end_size_yv.getValue(), Float.class);
			time_node.spin			= (float)Math.toRadians((Double)spin_v.getValue());
			time_node.spin_end		= (float)Math.toRadians((Double)end_spin_v.getValue());
			...
			time_node.end_size		= end_size.isSelected();
			time_node.end_spin		= end_spin.isSelected();
		...
		public String getListName()
		{
			String ret = "[位置=" + (int)(time_node.position * 100) + "%" + "]";

			if (time_node.enable_alpha) {
				ret += "[透明度=" + (int)time_node.alpha + "]";
			}
			if (time_node.enable_size) {
				ret += "[缩放=" + (int)time_node.scale_x + "," + (int)time_node.scale_y + "]";
			}
			if (time_node.enable_spin) {
				ret += "[旋转=" + (int)Math.toDegrees(time_node.spin) + "]";
			}
			if (time_node.end_size) {
				ret += "[END缩放=" + (int)time_node.scale_end_x + "," + (int)time_node.scale_end_y + "]";
			}
			if (time_node.end_spin) {
				ret += "[END旋转=" + (int)Math.toDegrees(time_node.spin_end) + "]";
			}
			return ret;
		}
