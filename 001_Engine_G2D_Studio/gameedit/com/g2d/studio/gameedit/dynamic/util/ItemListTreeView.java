...
import com.g2d.studio.gameedit.ObjectTreeViewTemplateDynamic;
...
//	-------------------------------------------------------------------------------------------------------------------------------
	...
	public class ItemListGroup extends ObjectGroup<DItemList, TItemList>
	{
		...
		final public ItemListTreeView view;
		...
		public ItemListGroup(String name) {
			...
			this.view = ItemListTreeView.this;
		}
...
//				}
				IdNameDialog dialog = new IdNameDialog(
						ItemListTreeView.this, 
						ItemListTreeView.this, "输入ItemList名字！");
				Integer id = dialog.showDialog();
				String name = dialog.getInputName();
				if (id != null && name!=null && name.length()>0) {
					try {
						DItemList effect = new DItemList(ItemListTreeView.this, id, name);
						addNode(root, effect);
						getTree().reload(root);
					} catch (Exception err) {
						err.printStackTrace();
