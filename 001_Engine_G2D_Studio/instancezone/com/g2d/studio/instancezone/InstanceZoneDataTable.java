	...
	class DataTableModel extends AbstractTableModel
	{
		...
		private Object[][] as_rows() {
			...
			for (Entry<String, Object> e : set) {
				rows[r][0] = e.getKey();
				rows[r][1] = Number.class.isInstance(e.getValue()) ? 
...
//						Math.max(selected[0], selected[selected.length-1]));
		...
		public void setValueAt(Object value, int row, int col) {
			if (col == 2) {
				Object[][] rows = as_rows();
				try {
					if (rows[row][1].equals(ValueType.NUMBER)) {
	...
	class AddRowDialog extends AbstractOptionDialog<Entry<String, Object>>
	{
		...
		ListEnumEdit<ValueType> var_type = new ListEnumEdit<ValueType>(
				ValueType.class);
		...
		protected boolean checkOK() {
			try {
				if (!var_name.getText().isEmpty()) {
					if (var_type.getValue() == ValueType.NUMBER) {
