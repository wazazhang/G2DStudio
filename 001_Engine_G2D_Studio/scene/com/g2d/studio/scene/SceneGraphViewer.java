...
public class SceneGraphViewer extends AbstractDialog
{
	...
	public SceneGraphViewer(Component owner) 
	{
		...
		JToolBar tools = new JToolBar();
		{
			...
			btn.setToolTipText("生成场景图");
			btn.setIcon(Tools.createIcon(Res.icon_scene_graph));
			btn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					display_stage.genAllLinks();
				}
			});
			tools.add(btn);
		}
	...
	{
		display_stage = new SceneGraphStage(sg);
		display_object_panel.getCanvas().changeStage(display_stage);
	}
...
//	-------------------------------------------------------------------------------------------------------------
	...
	{
		private SceneGraphPanel panel;
		
		public SceneGraphStage(SceneGraph sg) {
			panel = new SceneGraphPanel(sg);
			addChild(panel);
		}
	...
	static class SceneGraphPanel extends Panel
	{
		...
		public void genAllLinks()
		{
			ArrayList<SceneGraphNode.SceneGraphPath> dpaths = new ArrayList<SceneGraphNode.SceneGraphPath>();
			ArrayList<SceneGraphNode> allnodes = new ArrayList<SceneGraphNode>(path_finder.getMap().getAllNodes());
			for (SceneGraphNode src : allnodes)
			{
				for (SceneGraphNode dst : allnodes)
				{
					try {
						WayPoint path = path_finder.findPath(src, dst);
						if (path != null && path.getNext() != null)
						{
							SceneGraphNode.SceneGraphPath dpath = new SceneGraphNode.SceneGraphPath();
							dpath.src = src;
							dpath.dst = dst;
							
							for (WayPoint p : path) 
							{
								SceneGraphNode	snode	= (SceneGraphNode)p.map_node;
								SceneFrame		snodef	= all_nodes.get(snode.scene_id);
								
								if (p.getNext() != null)				
								{
									SceneGraphNode		next_node 	= (SceneGraphNode)(p.getNext().map_node);
									LinkInfo 			link_tp 	= snode.getLinkTransport(next_node);
									SceneFrame.LinkTP 	this_link 	= snodef.tp_map.get(link_tp.this_scene_obj_name);
									SceneFrame			dnodef		= all_nodes.get(link_tp.next_scene_id);
									SceneFrame.LinkTP 	next_link 	= dnodef.tp_map.get(link_tp.next_scene_obj_name);
									link_tp.this_scene_obj_x = this_link.unit.x;
									link_tp.this_scene_obj_y = this_link.unit.y;
									if (next_link != null && next_link.unit != null) {
										link_tp.next_scene_obj_x = next_link.unit.x;
										link_tp.next_scene_obj_y = next_link.unit.y;
									}
									dpath.links.add(link_tp);
								}
							}
			...
			Studio.getInstance().getPlugin().genSceneGraph(dpaths);
...
//			-------------------------------------------------------------------------------------------------------------
			...
			class LinkTP extends Sprite
			{
				...
				@Override
				public Tip getTip() {
					tip.setText((unit.getID() + " -> " + tp.next_scene_id + ":" + tp.next_scene_object_id));
					return tip;
				}
