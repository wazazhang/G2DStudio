...
//
//	@Override
//	public boolean resetID(SceneNode s, int newID) {
//		return id_factory.resetID(s, newID);
//	}
//	
//	@Override
//	public boolean containsID(int newID) {
//		return id_factory.containsID(newID);
//	}
//	@Override
//	public Integer peekID() {
//		return id_factory.peekID();
//	}
	
	@Override
	public IDFactoryInteger<SceneNode> idf() {
		return id_factory;
	}
	...
	private void resetID(SceneNode node, int newID) {
		node.resetID(this, newID);
	}
...
//	}
	...
	public void changeSceneResource(SceneNode node) {
		ChooseSceneDialog dialog = new ChooseSceneDialog();
		CPJWorld world = dialog.showDialog();
		if (world!=null) {
			node.setResource(world);
			saveScene(scene_dir, node);
		}
	}
...
//	scene root node
	...
	class ChooseSceneDialog extends CPJResourceSelectDialog<CPJWorld>
	{
		private static final long serialVersionUID = 1L;
...
//	scene menu
	...
	class SceneMenu extends JPopupMenu implements ActionListener
	{
		...
		JMenuItem reset_id		= new JMenuItem("重设ID");
		JMenuItem rename_scene	= new JMenuItem("重命名");	
		JMenuItem change_res 	= new JMenuItem("替换资源");
		...
		public SceneMenu(SceneNode node)
		{
			...
			reset_id.addActionListener(this);
			delete_scene.addActionListener(this);

			this.add(change_res);		
			change_res.addActionListener(this);
		}
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			else if (e.getSource() == reset_id) 
			{
				IdNameDialog dialog = new IdNameDialog(
						SceneManager.this, SceneManager.this, "输入ID！");
			...
			else if (e.getSource() == rename_scene)
			{
				String new_name = JOptionPane.showInputDialog(
						SceneManager.this, "输入场景名字", node.getData().name);
			...
			{
				if (JOptionPane.showConfirmDialog(SceneManager.this,
						"确实要删除场景\"" + node.getName() + "\"！") == JOptionPane.YES_OPTION) {
					removeScene(node);
				}
			} 
			else if (e.getSource() == change_res)
			{
				changeSceneResource(node);
			}
		}
	}
