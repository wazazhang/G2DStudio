...
import com.g2d.studio.scene.scene_map.StudioScene;
	...
	public Vector<SceneUnit> getList() {
		...
		TreeSet<SceneUnit> ret = new TreeSet<SceneUnit>(new Comparator<SceneUnit>() {
			public int compare(SceneUnit o1, SceneUnit o2) {
				return CUtil.getStringCompare().compare(o1.getID(), o2.getID());
			}
		});
		for (SceneUnitTag tag : list) {
			try {
...
//			st.setVisible(true);
			Studio.getInstance().getPlugin().openSceneScriptEditor(scene_node);
		}
			...
			String ret = JOptionPane.showInputDialog(this, 
					"输入0～1的小数", 
					StudioScene.default_mask_alpha + "");
			try {
				if (ret != null) {
					float v = Float.parseFloat(ret.trim());
					if (v >= 0 && v <= 1f) {
						StudioScene.default_mask_alpha = v;
					} else {
			...
			String ret = JOptionPane.showInputDialog(this, 
					...
					Integer.toString(StudioScene.default_mask_color, 16));
			try {
				if (ret != null) {
					int v = Integer.parseInt(ret.trim(), 16);
					StudioScene.default_mask_color = v;
				}
			} catch (Throwable ex) {
				JOptionPane.showMessageDialog(this, "输入错误!");
			}
	...
	class SceneStage extends Stage
	{
		...
		public void update() {
			...
			if (getRoot().isMouseDown(MouseEvent.BUTTON1)) {
				...
				sb.append("场景:("+
							scene_container.getWorld().getMouseX()+","+
							scene_container.getWorld().getMouseY()+") ");
				if (getSelectedUnit()!=null) {
					sb.append("单位:("+
							getSelectedUnit().getGameUnit().getIntX()+","+
							getSelectedUnit().getGameUnit().getIntY()+") ");
				}
...
//	-----------------------------------------------------------------------------------------------------------------------------
	...
	CUtil.ICompare<SceneUnitTag<?>, SceneUnitTag<?>>
	{
		...
		{
			this.setWorld(new StudioScene(this, scene_resource, scene_world.name));
		...
		protected void renderAfter(Graphics2D g)
		{
			...
			Drawing.drawStringBorder(g, 
					"FPS="+getRoot().getFPS() + 
					" - 渲染次数(" + Profiling.getAllNodeRenderTime(this)+")", 
					1, 1, 0);
		}
...
//					}
		...
		public CellSprite getToolSprite() {
			if (SelectUnitTool.getUnitTool().isVisible()) {
				if (getSelectedPage() == this) {
					XLSUnit xls_unit = SelectUnitTool.getUnitTool().getSelectedUnit();
					if (xls_unit!=null) {
			...
			if (cspr != null)
			{
				...
				scene.setAlpha(g, 0.75f);
				{
					...
					Drawing.drawStringBorder(g, 
							...
							0, (int)cspr.getSprite().getVisibleBotton() + 1, 
...
//					}
		...
		public CellSprite getToolSprite() {
			if (SelectUnitTool.getUnitTool().isVisible()) {
				...
				if (getSelectedPage() == this && cpj != null) {
					if (cpj != selected_sprite.getKey()) {
						selected_sprite.setKey(cpj);
						selected_sprite.setValue(cpj.createCellSprite());
					}
