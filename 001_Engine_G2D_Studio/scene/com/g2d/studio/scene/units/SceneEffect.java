...
//			}
			if (editor.getSelectedPage().isSelectedType(getClass())) 
			{
				...
				if (particles!=null && particles.getData().bounding_box != null) {
					g.setColor(Color.GRAY);
					g.draw(particles.getData().bounding_box);
				}
	...
	public boolean isInCamera()
	{
		if (getOwnerScene() != null) 
		{
			if (particles != null)
			{
				Rectangle bounding = particles.getData().bounding_box;
				if (bounding != null) {
					if (CMath.intersectRect2(
							(float)getOwnerScene().getCameraX(), 
							(float)getOwnerScene().getCameraY(), 
							(float)getOwnerScene().getCameraWidth(), 
							(float)getOwnerScene().getCameraHeight(),
							(float)x + bounding.x,
							(float)y + bounding.y, 
							bounding.width, 
							bounding.height)) {
