	...
	public static Cursor[] createSpriteCursor(String name, CSprite cspr, int anim)
	{
		...
		for (int f = 0; f < frame_count; f++) {
			...
			Cursor cur = Tools.createCustomCursor(img, 
					new Point((int)-bounds.X1, (int)-bounds.Y1), 
					name + "_" + anim + "_" + f);
			cursors[f] = cur;
//			System.out.println(name + "_" + anim + "_" + f);
	...
	public static BufferedImage getFrameSnapshot(CSprite cspr, int anim, int frame)
	{
		...
		BufferedImage ret = Tools.createImage((int)border.getWidth(), (int)border.getHeight());
...
//	--------------------------------------------------------------------------------
	...
	static public BufferedImage createImage(CSprite spr, int anim, int frame)
	{
		...
		{	
			...
			BufferedImage buf = gc.createCompatibleImage((int)bounds.getWidth(), (int)bounds.getHeight(), Transparency.TRANSLUCENT);
			Graphics2D g = (Graphics2D)buf.createGraphics();
			IGraphics ig = wrap(g);
			try {
	...
	static public BufferedImage combianImage(int width, int height, ImageAnchor anchor, Image ... args)
	{
		try
		{
			BufferedImage dst = createImage(width, height);
			Graphics2D g = (Graphics2D)dst.createGraphics();
			for (Image img : args) {
				Drawing.drawImageAnchor(g, img, anchor, 0, 0, width, height);
			}
			g.dispose();
			return dst;
		}
