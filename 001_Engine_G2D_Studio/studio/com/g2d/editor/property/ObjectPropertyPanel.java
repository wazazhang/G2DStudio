			...
			CellEditAdapter<?> ... adapters)
	{
		...
		// center
		{
			...
			
			{
				...
				anno_text.setMinimumSize(new Dimension(200, 100));
...
//		----------------------------------------------------------
		private void cleanValue() {
			try {
				...
				try {
					PropertyNotNull pp = column.getAnnotation(PropertyNotNull.class);
					if (pp == null) {
						Object fdata = Parser.getZeroValue(column.getType());
						column.set(object, fdata);
						setValueAt(fdata, getSelectedRow(), 1);
					}
				} catch (Exception err) {}
				onFieldChanged(object, column);
				this.repaint();
			} catch (Exception err) {
				err.printStackTrace();
			}
		...
		private void paste() {
			try {
				...
				if (copy_field_data != null) {
					...
					try {
						PropertyNotNull pp = column.getAnnotation(PropertyNotNull.class);
						if (pp != null && copy_field_data == null) {
...
//	--------------------------------------------------------------------------------------------------------------------------------------
	...
	class  ValueEditor extends DefaultCellEditor implements TableCellEditor
	{
		...
		public boolean stopCellEditing() {
			if (current_cell_edit != null) {
				current_cell_edit.commitValue();
			}
