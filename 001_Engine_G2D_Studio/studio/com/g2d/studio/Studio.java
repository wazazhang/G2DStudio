...
import com.g2d.studio.StudioPlugin.XLSDataSource;
...
//	final private FileOutputStream	project_lock;
	...
	final public XLSDataSource xls_tplayer;
	final public XLSDataSource xls_tunit;
	final public XLSDataSource xls_tpet;
	final public XLSDataSource xls_titem;
	final public XLSDataSource xls_tshopitem;
	final public XLSDataSource xls_tskill;
...
//	final public File				plugins_dir;
	...
	private Studio(File g2d_file, IO io, String[] args) throws Throwable
	{
		instance 			= this;
		this.io				= io;
		...
		project_dir 		= io.createFile(project_file.getParent());
		...
		Builder.setBuilder(
				studio_plugin.createResourceBuilder(project_dir));
...
//		}
		...
		xls_tplayer 	= studio_plugin.GetPlayerXLS();//getOrCreateXlsFile(StudioConfig.XLS_TPLAYER);
		xls_tunit 		= studio_plugin.GetUnitXLS();//getOrCreateXlsFile(StudioConfig.XLS_TUNIT);
		xls_tpet 		= studio_plugin.GetPetXLS();//getOrCreateXlsFile(StudioConfig.XLS_TPET);
		xls_titem 		= studio_plugin.GetItemXLS();//getOrCreateXlsFile(StudioConfig.XLS_TITEM);
		xls_tshopitem 	= studio_plugin.GetShopItemXLS();//getOrCreateXlsFile(StudioConfig.XLS_TSHOPITEM);
		xls_tskill 		= studio_plugin.GetSkillXLS();//getOrCreateXlsFile(StudioConfig.XLS_TSKILL);
...
//		    }
		...
		System.out.println(System.setProperty("user.dir", project_dir.getPath()));
...
//			tool_bar.add(btn);
	...
	{
		return io.createFile(project_dir, path);
	}
...
//	}
	...
	{
		File file = io.createFile(project_dir, path);
		if (!file.exists()) {
...
//	main entry
	...
	static public void start_studio(String[] args) 
	{
		try
		{
			...
			{
				String usage = "usage : g2dstudio.jar [g2d file path]";
				System.err.println();
			...
			{			
				...
				new AwtEngine();
				IO io = new FileIO(args);
				System.out.println("Open: " + io.createFile(args[0]).toString());
				File g2d_file = io.createFile(args[0]);
				new Studio(g2d_file, io, args);
			}
		}
		catch (Throwable e)
		{
