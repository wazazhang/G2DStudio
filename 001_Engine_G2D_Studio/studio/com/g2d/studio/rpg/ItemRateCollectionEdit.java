...
import com.cell.rpg.template.ability.ItemRateCollection.ItemDrop;
...
//	--------------------------------------------------------------------------------------------------------------
	...
	public ItemRateCollectionEdit(Component owner, ItemRateCollection src_data) 
	{
		...
		pnl_items_ = new ListPanel("道具列表", edit_data.GetItemDropList());
		this.add(pnl_items_);
	}
...
//	--------------------------------------------------------------------------------------------------------------
	...
	class ListPanel extends JPanel implements ActionListener
	{
		...
		public ListPanel(String title, Vector<ItemDrop> drops)
		{
			...
			this.list_data = new Vector<ListItemData>(drops.size());
			for (ItemDrop key : drops){
				Integer count = key.count;
				Integer rate = key.rates;
				XLSItem item = Studio.getInstance().getObjectManager().getObject(XLSItem.class, key.item_id);
				if (item != null) {
