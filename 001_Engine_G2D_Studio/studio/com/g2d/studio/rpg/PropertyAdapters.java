...
import com.g2d.studio.quest.group.QuestGroupSelectCellEdit;
...
import com.g2d.studio.talks.TalkSelectCellEdit;
		...
		public PropertyCellEdit<?> getCellEdit(ObjectPropertyEdit owner,
			...
			try{
				if (adapter != null) {
					...
					case QUEST_GROUP_ID: {
						QuestGroupSelectCellEdit dialog = new QuestGroupSelectCellEdit(
								owner.getComponent(), (Integer)fieldValue);
						dialog.showDialog();
						return dialog;
					}
					case SKILL_ID:{
					...
					case NPC_TALK_SCRIPT_NAME: {
						TalkSelectCellEdit talkselect = new TalkSelectCellEdit(
								owner.getComponent(), 
								(String)fieldValue);
						talkselect.showDialog();
						return talkselect;
					}
				...
				DefaultTableCellRenderer src) 
		{
			...
			try {
				if (adapter != null) {
					...
					case QUEST_GROUP_ID: {
						if (fieldValue != null) {
							DQuestGroup qg = Studio.getInstance().getQuestGroupManager().getQuestGroup((Integer) fieldValue);
							if (qg != null) {
								src.setText(qg.getName());
								src.setIcon(qg.getIcon(false));
							}
						}
						break;
					}
					...
					case NPC_TALK_SCRIPT_NAME: {
						TalkFile tf = Studio.getInstance().getTalkManager().getTalk(
								(String)fieldValue);
						if (tf != null) {
							src.setText(tf.getName());
							src.setIcon(tf.getListIcon(false));
						}
						break;
					}
					case ITEM_PROPERTY_SAVED_TYPE: {
