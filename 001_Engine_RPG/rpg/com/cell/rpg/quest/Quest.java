...
//	base
	...
	@Property("任务完成时，自动接收下一个任务")
	@PropertyAdapter(PropertyType.QUEST_ID)
	public Integer				next_quest;
//	----------------------------------------------------------------------------------------------------------------
//	quest
...
//	----------------------------------------------------------------------------------------------------------------
	...
	static {
		QuestAbilityManager qam = QuestAbilityManager.getManager();
		if (qam != null) {
			Class<?>[] set = qam.getQuestExtAbilities();
			if (set != null) {
				static_abilities = CUtil.link(static_abilities, set);
			}
		}
	}
}
