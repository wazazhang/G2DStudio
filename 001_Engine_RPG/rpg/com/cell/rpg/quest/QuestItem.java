...
//	--------------------------------------------------------------------------------------
	...
	public Class<?>[] getSubAbilityTypes() {
		if (!is_result) {
			return static_tag_abilities;
		} else {
			return static_result_abilities;
		}
	}
...
//	--------------------------------------------------------------------------------------
	...
	@Property("[条件] 依赖已完成的任务组")
	final public static class TagQuestGroup extends Tag
	{
		private static final long serialVersionUID = 1L;

		@Property("已完成的任组务ID")
		@PropertyAdapter(PropertyType.QUEST_GROUP_ID)
		public int quest_group_id	= -1;
...
//	unit group 
	...
	final public static class TagBattleWin extends Tag
	{
		private static final long serialVersionUID = 1L;
...
//	----------------------------------------------------------------------------
	...
	private static Class<?>[] static_tag_abilities = new Class<?>[] {
		TagItem.class,
		TagQuest.class,
		TagQuestGroup.class,
		TagQuestItem.class,
	...
	private static Class<?>[] static_result_abilities = new Class<?>[] {
		AwardItem.class,
		DropItem.class,
		AwardTeleport.class,
		AwardBattle.class,
	...
	static {
		QuestAbilityManager sam = QuestAbilityManager.getManager();
		if (sam != null) {
			Class<?>[] sett = sam.getQuestTagExtAbilities();
			if (sett != null) {
				static_tag_abilities = CUtil.link(static_tag_abilities, sett);
			}
			Class<?>[] setr = sam.getQuestResultExtAbilities();
			if (setr != null) {
				static_result_abilities = CUtil.link(static_result_abilities, setr);
			}
		}
	}
