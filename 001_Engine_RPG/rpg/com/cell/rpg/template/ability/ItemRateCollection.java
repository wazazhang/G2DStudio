...
public class ItemRateCollection extends AbstractAbility
{
	...
	public Vector<ItemDrop> GetItemDropList()
	{
		Vector<ItemDrop> ret = new Vector<ItemDrop>(item_ids_.size());
		if (item_ids_.size() > 0)
		{
			int i = 0;
			for ( Integer id : item_ids_ )
			{
				ItemDrop dp = new ItemDrop();
				dp.item_id = item_ids_.get(i);
				dp.count = item_counts_.get(i);
				dp.rates = item_rates_.get(i);
				ret.add(dp);
				++i;
			}
		}
