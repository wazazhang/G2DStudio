...
public class FlashCrossdomainService extends IoHandlerAdapter
{
	static public void main(String[] args) 
	{
		...
		if (args.length == 3) {
			...
			String req = CFile.readText(new File(args[1]));
			String res = CFile.readText(new File(args[2]));
			new FlashCrossdomainService(port, req, res);
		} else {
	...
	protected String REQUST = 
			CIO.readAllText("/com/net/flash/request.xml");
	
	protected String RESPONSE = 
			CIO.readAllText("/com/net/flash/crossdomain.xml");
	...
	public FlashCrossdomainService(int port, String request, String response)
	{
		System.out.println("starting flash crossdomain service at port : " + port);
		if (request != null) {
			REQUST = request.trim().toLowerCase();
		}
		if (response != null) {
	...
	public void messageReceived(final IoSession session, final Object message) throws Exception {
		...
		String msg = message.toString().trim().toLowerCase();
		if (REQUST.equals(msg)) {
			session.write(RESPONSE + "\0");
		}
	}
