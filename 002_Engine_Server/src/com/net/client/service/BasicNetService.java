...
//	notifiers
	...
	final private ConcurrentHashMap<Protocol, Message>
									UnhandledMessages 		= new ConcurrentHashMap<Protocol, Message>();
...
//	}
     ...
     * @return
     */
	final public AtomicReference<Message> sendRequest(Message message, long timeout, WaitingListener<?, ?> ... listeners)
    {
     ...
     * @param listeners
     */
    final public AtomicReference<Message> sendRequest(Message message, WaitingListener<?,?> ... listeners) 
    {
...
//	notify
	 ...
	 * @param listener
	 */
	final public void registNotifyListener(Class<? extends Message> message_type, NotifyListener<?> listener) 
	{
		...
		if (!UnhandledMessages.isEmpty()) {
			...
			for (Entry<Protocol, Message> unotify : UnhandledMessages.entrySet()) {
	 ...
	 * @param listener
	 */
	final public void unregistNotifyListener(Class<? extends Message> message_type, NotifyListener<?> listener) 
	{
		synchronized (notifies_lock){
			HashSet<NotifyListener<?>> notifys = notifies_map.get(message_type);
			if (notifys != null) {
	 ...
	 * @param message_type
	 */
	final public void clearNotifyListener(Class<? extends Message> message_type) 
	{
		synchronized (notifies_lock)
		{
...
//    -------------------------------------------------------------------------------------------------------------------
	...
	final private void processReceiveSessionMessage(ServerSession session, Protocol protocol, Message message) 
	{
		try {
	...
	final private void processReceiveChannelMessage(ServerSession session, Protocol protocol, Message message) 
	{
		try {
			onReceivedMessage(session, message);
		} catch (Exception err) {
...
//	----------------------------------------------------------------------------------------------------------------------------
	...
	private boolean tryReceivedNotify(Message message) 
	{
		synchronized (notifies_lock)
		{
	...
	private boolean tryReceivedResponse(Protocol protocol, Message message)
	{
	...
	private boolean tryPushUnhandledNotify(Protocol protocol, Message message)
	{
...
//	----------------------------------------------------------------------------------------------------------------------------
	...
	protected void onListeningRequest(Message request, WaitingListener<?,?> listeners){}
	...
	protected void onReceivedMessage(ServerSession session, Message message){}
	
	protected void onSentMessage(ServerSession session, Message message){}
//	----------------------------------------------------------------------------------------------------------------------------
	...
	private class SimpleClientListenerImpl implements ServerSessionListener
	{
		...
		private class ReceiveTask implements Runnable
		{
			...
			public ReceiveTask(ServerSession session, Protocol protocol, Message message)
			{
		...
		private class ReceiveChannelTask implements Runnable
		{
			...
			public ReceiveChannelTask(ServerSession session, Protocol protocol, Message message) 
			{
				this.message = message;
				this.session = session;
				this.protocol = protocol;
...
//	-------------------------------------------------------------------------------------------------
	...
	@SuppressWarnings("unchecked")
	private class Request extends AtomicReference<Message> implements Runnable
	{
		...
		private Request(Message msg, long timeout, WaitingListener ... listeners)
		{
...
//			System.out.println("request " + this);
		...
		private void messageResponsed(Protocol protocol, Message response) 
		{
			set(response);
